...
import '../../main.dart';
import '../components/auth_credential.dart';
import '../components/nfc/apdu/exceptions.dart';
    ...
    useEffect(() {
      NfcManager.instance.startSession(
          ...
          onDiscovered: (card) async {
            ...
            try {
              final cie = MRTDInterface(NFCCard(
                  ({required Uint8List data}) => isoDep.transceive(data)));

              stepsDescription.value = {'Authentication': -1};
              steps.value = ['Authentication'];
              await cie.authenticate(MRZAuthentication(
                  birthStr: mrz.doB, expireStr: mrz.doE, docNoStr: mrz.docNo));
